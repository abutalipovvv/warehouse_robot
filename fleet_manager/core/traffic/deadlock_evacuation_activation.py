...
class EvacuationActivationMixin:
    ...
    def _activate_deadlock_evacuation(
        ...
        recovery_latch_key: tuple[tuple[str, ...], str, str, int] | None,
        now: float,
    ) -> str:
        """Install the chosen escape, detour transaction or reverse retreat."""
        robot = candidate.robot
        target_clock = candidate.target_clock
        target_lm = candidate.target_lm
        retreat_is_noop_at_current_lm = (
            candidate.retreat_is_noop_at_current_lm
        )
        graph_escape_route = candidate.graph_escape_route
        portal_blocked_edges = candidate.portal_blocked_edges
