...
from fleet_manager.manager.state import FleetState, PlanningState, RecoveryState


class ReplanningService:
    """Own common runtime-replan state transitions without a manager reference."""
    ...
    def state_is_current(
        ...
        *,
        ...
        order = self._fleet.task_manager.orders.get(
        ...
        return bool(
            ...
            == self._safe_start_lm(robot)
            and (allowed_stages is None or stage in allowed_stages)
        )

    @staticmethod
    def is_same_transaction(
    ...
    def install_transaction(
        ...
        *,
        ...
        self._planning.runtime_replans[robot.name] = state
    ...
    def coupled_failure_count(self, robot_names: Collection[str]) -> int:
        ...
        return max(
            (
                int(count)
                for key, count in self._recovery.coupled_replan_failures.items()
                if members.issubset(set(key))
            ),
            default=0,
        )

    def coupled_latest_attempt(self, robot_names: Collection[str]) -> float:
        members = {str(name) for name in robot_names if str(name)}
        if not members:
            return 0.0
        return max(
            (
                ...
                in self._recovery.coupled_replan_last_attempt.items()
                if members.issubset(set(key))
            ),
            default=0.0,
        )

    def clear_coupled_attempts(
        self,
        robot_names: Collection[str],
        *,
        ...
        if not members:
            return
        for state in (
            self._recovery.coupled_replan_last_attempt,
            self._recovery.coupled_replan_failures,
        ):
            for key in list(state):
                key_members = set(key)
                if members.issubset(key_members) or (
                    include_subsets and key_members.issubset(members)
                ):
                    state.pop(key, None)
...
class RuntimeReplanMixin:
    ...
    def _runtime_replan_state_is_current(
        ...
        *,
        ...
        return self._replanning_service.state_is_current(
            robot,
            state,
    ...
    def _coupled_replan_failure_count_for_members(
        ...
        return self._replanning_service.coupled_failure_count(
            robot_names
        )
    ...
    def _coupled_replan_latest_attempt_for_members(
        ...
        return self._replanning_service.coupled_latest_attempt(
            robot_names
        )
    ...
    def _clear_coupled_replan_attempts_for_members(
        ...
        *,
        ...
        self._replanning_service.clear_coupled_attempts(
            robot_names,
