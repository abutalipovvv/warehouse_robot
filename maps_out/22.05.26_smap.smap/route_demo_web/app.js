(function () {
  ...
  class GeometryService {
    ...
    interpolateAngle(start, goal, t) {
      const delta = this.normalizeAngle(goal - start);
  ...
  class GraphModel {
    constructor(landmarks, edges, routeCatalog) {
    ...
    getRoute(startName, goalName) {
      return this.routeCatalog[`${startName}|${goalName}`] || null;
    }
  }
  ...
  class MapRenderer {
    constructor(dom, graphModel, geometry) {
      this.dom = dom;
      this.graphModel = graphModel;
      this.geometry = geometry;
    }
    ...
    drawRobotAt(pathNodes) {
      ...
      const current = this.graphModel.nodeByName.get(pathNodes[0]);
      const next = pathNodes.length > 1 ? this.graphModel.nodeByName.get(pathNodes[1]) : null;
  ...
  class RouteDemoApp {
    constructor(data) {
      ...
      this.trajectoryPlanner = new TrajectoryPlanner(this.graphModel, this.geometry);
      this.renderer = new MapRenderer(this.dom, this.graphModel, this.geometry);
    ...
    planRoute() {
      ...
      this.currentRoute = this.graphModel.getRoute(startName, goalName);
