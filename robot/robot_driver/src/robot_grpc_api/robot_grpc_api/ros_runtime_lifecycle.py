...
def _slam_toolbox_share() -> Path:
    """Resolve slam_toolbox from an installed overlay or this source tree."""
    try:
        from ament_index_python.packages import get_package_share_directory

        return Path(get_package_share_directory("slam_toolbox")).resolve()
    except Exception:
        robot_root = Path(__file__).resolve().parents[4]
        return robot_root / "ros2_libs" / "src" / "slam_toolbox"
