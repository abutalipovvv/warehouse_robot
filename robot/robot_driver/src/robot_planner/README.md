...
Inputs are `robot_status`, `odom`, `initialpose`, and route services. Output is
`motion/route_cmd_vel`; `robot_motion_gateway` owns the final driver `cmd_vel`.
A route is allowed to finish only when both position and final yaw are stable
within configured tolerances. `route/executor_state` and the
