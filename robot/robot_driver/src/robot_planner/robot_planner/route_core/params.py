...
def discover_default_params_path() -> Path:
    ...
    for parent in module_path.parents:
        if not (
            (parent / "robot_planner").exists()
            and (parent / "robot_map_manager").exists()
        ):
            continue
        for candidate in (parent / "params" / "params.yaml", parent / "params.yaml"):
            if candidate.is_file():
                return candidate.resolve()

    for candidate in (Path.cwd() / "params" / "params.yaml", Path.cwd() / "params.yaml"):
        if candidate.is_file():
            return candidate.resolve()
    if len(module_path.parents) > 3:
        return module_path.parents[3] / "params" / "params.yaml"
    ...
    if isinstance(nav2_overrides, dict):
        ...
        for costmap_name in ("local_costmap", "global_costmap"):
            ...
            if not isinstance(costmap, dict):
                continue
            footprint = _parse_nav2_footprint(costmap.get("footprint"))
            if footprint:
                robot_model["footprint"] = footprint
                robot_model["nav2_costmap"] = costmap_name
                return params
            if "robot_radius" not in costmap:
                continue
...
DEFAULT_ROUTE_PARAMS: dict[str, Any] = {
    ...
    "navigation": {
        ...
        "curve_speed_limit": 0.22,
        "curve_heading_threshold_deg": 5.0,
        "curve_curvature_threshold": 0.05,
        "curve_preview_distance": 0.25,
        "curve_angular_reserve": 0.35,
        "curve_min_speed": 0.08,
        "rejoin_speed_limit": 0.16,
        "hard_rejoin_speed_limit": 0.06,
        "strict_edge_tracking": True,
        "strict_speed_limit": 0.40,
        "edge_transition_tolerance": 0.04,
        "drive_alignment_angle_deg": 12.0,
        "lateral_error_gain": 3.0,
        "rejoin_lateral_error_gain": 1.5,
        "precision_lateral_tolerance": 0.02,
        "precision_speed_limit": 0.18,
