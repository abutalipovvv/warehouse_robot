...
class RobotRouteNode(Node):
    ...
    def _control_step(self) -> None:
        ...
        if self._route_mode_active and not self._command_is_active():
            self._publish_motion_mode("IDLE", "route finished")
            self._route_mode_active = False
    ...
    def _on_initial_pose(self, message: PoseWithCovarianceStamped) -> None:
        ...
        self._publish_cmd_vel(0.0, 0.0)
        self._publish_motion_mode("IDLE", "pose relocated")
        self._route_mode_active = False
        self.get_logger().info(
    ...
    def _handle_execute_route(self, request, response):
        try:
            ...
            self._publish_motion_mode("ROUTE", f"executing {route.route_id}")
            self._route_mode_active = True
    ...
    def _handle_cancel_route(self, request, response):
        try:
            ...
            self._publish_cmd_vel(0.0, 0.0)
            self._publish_motion_mode("IDLE", message)
            self._route_mode_active = False
    ...
    def _handle_load_map(self, request, response):
        try:
            ...
            self._publish_cmd_vel(0.0, 0.0)
            self._publish_motion_mode("IDLE", "map changed")
            self._route_mode_active = False
    ...
    def _publish_motion_mode(self, mode: str, reason: str) -> None:
        ...
        message.data = json.dumps(
            {"mode": str(mode), "reason": str(reason)},
            separators=(",", ":"),
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cmd-vel-topic", default="motion/route_cmd_vel")
