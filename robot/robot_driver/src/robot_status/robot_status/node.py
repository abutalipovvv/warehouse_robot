...
class RobotStatusNode(Node):
    ...
    def _on_executor_state(self, message: ExecutorState) -> None:
        ...
        if route_was_active and not route_is_active:
            # Route execution and teleoperation share the gateway's final
            # cmd_vel output. The last non-zero PID command must not be
            # mistaken for fresh manual control during ARRIVED.
