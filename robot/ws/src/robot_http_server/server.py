...
ROUTE_CORE_ROOT = PROJECT_ROOT / "robot_planner" / "robot_planner"
if ROUTE_CORE_ROOT.exists():
    route_core_root_str = str(ROUTE_CORE_ROOT)
    if route_core_root_str not in sys.path:
        sys.path.insert(0, route_core_root_str)
...
DEFAULT_ROBOT_MAP_DIR = PROJECT_ROOT / "robot_map_manager" / "maps_out" / "22.05.26_smap.smap"
...
def resolve_map_dir(map_dir: Path) -> Path:
    ...
    candidates = [
        ...
        PROJECT_ROOT / "robot_map_manager" / map_dir,
        PROJECT_ROOT / "robot_map_manager" / "maps_out" / safe_name,
        PROJECT_ROOT / "map_data" / map_dir,
    ]
    if not safe_name.endswith(".smap"):
        candidates.append(PROJECT_ROOT / "robot_map_manager" / "maps_out" / f"{safe_name}.smap")
