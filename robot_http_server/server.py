...
from route_core import build_editable_map_bundle_payload, build_editable_map_payload, list_editable_maps, load_route_params, save_editable_map
...
class RobotHttpApiBridge:
    ...
    def site_data_script(self) -> str:
        site_payload = json.dumps(self.ros_client.site_payload(), ensure_ascii=False).replace(
            "</script>",
            "<\\/script>",
        )
        return f"window.ROBOT_WEB_DATA = {site_payload};\n"
    ...
    def push_map_payload(self, payload: dict[str, Any]) -> dict[str, Any]:
        active = self.ros_client.active_map_payload()
        active_map_dir = Path(str(active.get("mapDir") or "")).resolve()
        source_name = str(payload.get("sourceMapName") or payload.get("mapName") or "").strip()
...
class RobotWebRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        try:
            ...
            if path == "/demo-data.js":
                self._send_script(self._require_bridge().site_data_script())
                return
    ...
    def _send_error_json(self, status: int, message: str) -> None:
        encoded = json.dumps({"ok": False, "error": str(message)}, ensure_ascii=False).encode("utf-8")
