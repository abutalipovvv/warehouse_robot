"""Persist parameters and apply declared Nav2 runtime values."""
...
class RosRuntimeParametersMixin:
    ...
    def save_params_payload(self, params_payload: dict[str, Any], *, reload_runtime: bool = True) -> dict[str, Any]:
        ...
        reloaded = False
        if reload_runtime:
            # Robot planner and status nodes watch params.yaml and reload it on
            # their next control/status tick. Reloading the active map here is
            # both unnecessary and harmful because it resets localization.
            reloaded = True
            try:
